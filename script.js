...
// FIXED: Complete PDF generation function
...
Account Name: ${JSON.parse(localStorage.getItem('fxTaeCurrentUser') || '{}').name || 'Trader'}
...
2. ${trades.length > 0 && (trades.filter(t => t.pnl > 0).length / trades.length * 100) < 50 ? 'Focus on improving win rate through better trade selection.' : 'Excellent win rate! Maintain consistency.'}
...
function setupDownloadButtons() {
    // Setup download buttons for Today's P&L card
    ...
    // Setup download buttons for Trades card
    const tradesDownloadBtn = document.querySelector('.stat-card.purple-card .download-btn');
    if (tradesDownloadBtn) {
        tradesDownloadBtn.addEventListener('click', downloadTodayStats);
    }
    
    // Setup download buttons for Weekly Performance card
    ...
    // Setup download buttons for Monthly Performance card
    ...
    const chartDownloadBtns = document.querySelectorAll('.chart-download-btn .btn-secondary');
    chartDownloadBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            const chartId = this.closest('.chart-card').querySelector('canvas').id;
            const chartName = this.closest('.chart-card').querySelector('h3').textContent;
            downloadChartAsPDF(chartId, chartName);
        });
    });
}
